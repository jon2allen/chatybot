...
class JsonlSessionStore(BaseSessionStore):
    ...
    def list_sessions(
        ...
        for d in os.listdir(self.sessions_dir):
            ...
            if os.path.isdir(dpath) and not d.startswith("."):
                ...
                if not meta:
                    continue
                if model_filter and model_filter not in (meta.get("model_alias") or "").lower():
                    continue

                compressed = bool(
                    meta.get("compressed") or os.path.exists(self._turns_gz_path(d))
                )
                if compressed_filter is not None and compressed != compressed_filter:
                    continue
    ...
    def merge_sessions(self, target_name: str, source_targets: List[str]) -> str:
        ...
        self.create_session(
            ...
            initial_prompt=first_slug or "merged_session",
            notes=merged_notes,
        )

        turns_file = self._turns_path(new_sid)
        ...
        meta = self._read_meta(new_sid) or {}
        meta["turn_count"] = len(merged_turns)
        self.save_meta(new_sid, meta)
