...
def validate_and_route(invocation: dict[str, Any], config: dict[str, Any]) -> tuple[Any, list, dict]:
    ...
    try:
        ...
        raise ImportError(f"Failed to import tool '{tool_name}': {e!s}")
...
def main():
    ...
    try:
        ...
        write_json_response({
            "status": "error",
            "message": f"Malformed payload JSON structure: {exc!s}",
            "code": "INVALID_JSON"
        }, status_code=1)
    ...
    try:
        ...
        write_json_response({
            "status": "error",
            "tool": invocation_data["tool"],
            "message": f"Runtime execution exception triggered: {runtime_exc!s}",
