...
| `/会话 查询` | 会话 | `/会话 查询 <关键词> [标识] [完整] [since=N] [var=V]` | 搜索会话轮次、笔记和草稿区 |
| `/会话 获取` | 会话 | `/会话 获取 <sid> [turn=N] [part=prompt\|response\|both]` | 从过去或当前会话中提取文本 |
| `/reloadmacros` | 核心 | `/reloadmacros [宏定义文件路径]` | 重新加载并更新所有的宏模板定义 |
| `/询问` | 交互 | `/询问 [是否\|选择] "问题" [选项] [-> 变量]` | 提示用户输入（非TTY标准输入时自动跳过） |
| `/文档` | 核心 | `/文档 [文件名\|搜索 关键词\|食谱\|路径] [page=N] [var=V]` | 列出、查看、搜索或加载内置文档 |
...
| `/保存` | 文件 | `/保存 文件路径 [all] [withthink\|raw\|nothink]` | 将上一次的模型回复内容保存到文件（默认忽略思考过程；使用withthink/raw保留） |
...
| `/工具 草稿` | 工具 | `/工具 草稿 [on\|off\|清空\|status]` | 切换/管理模型专用临时草稿工作区 (scratchpad) |
...
- `/文件 文件路径` - 加载文本到缓冲区中
- `/文件库1-5` - 辅助持久化文件缓存区管理
- `/保存 文件路径 [all] [withthink|raw|nothink]` - 保存上一次模型回复到磁盘文件（默认忽略思考过程）
- `/多行输入` - 开启复杂的换行提示词录入
