...
class HelpSystem:
    ...
    def _initialize_commands(self) -> None:
        ...
        self.register_command(CommandHelp(
            ...
            usage="/tool [on|off|list|enable <tool>|disable <tool>|rate_limit <seconds>|prompt|loop [turns|max|max=val] [force]|auto [on|off]|<json_file.json>|<json_invocation>]",
            ...
            examples=["/tool on", "/tool off", "/tool list", "/tool rate_limit 3", "/tool enable list_directory", "/tool disable write_file", "/tool disable all", "/tool prompt", "/tool loop", "/tool auto on", '/tool {"tool": "list_directory", "arguments": {"path": "."}}'],
            see_also=["/run"]
        ))
