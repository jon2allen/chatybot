...
class HelpSystem:
    ...
    def _initialize_commands(self) -> None:
        ...
        self.register_command(CommandHelp(
            ...
            usage="/tool [on|off|list [detail] [pattern] [var=<name>]|enable <tool>|disable <tool>|scratch [on|off|clean|status]|append_mode [off|full|summary]|max_turns <int>|rate_limit <seconds>|prompt [live_edit|edit_live|restore]|retry [edit|fix|run] [--continue]|continue [max_turns]|inject [file=<path>|<payload>]|loop [turns|max|max=val] [force]|auto [on|off]|history [<turn_id>|current] [csv [file.csv]] [var=<name>]|export csv <file.csv>|replay [<turn_id>] [at <N>|diff <A> <B>|step] [limit=<N>] [var=<name>]|<json_file.json>|<json_invocation>]",
            long_desc="Manage tool mode for LLM tool calling. When tool mode is on, tool definitions from tools_config.toml are available for the LLM to use.\n\nSubcommands:\n  /tool on - Enable tool mode, inject tool definitions into context\n  /tool off - Disable tool mode\n  /tool list [detail] [pattern] [var=<name>] - List all tools, showing enabled/disabled status and description. Populates protected variable ${TOOL_LIST} and optional target variable\n  /tool enable <tool>|all - Enable a specific tool or all tools at runtime\n  /tool disable <tool>|all - Disable a specific tool or all tools at runtime\n  /tool scratch [on|off|clean|status] - Toggle or manage the dedicated temporary scratchpad area for disposable Python/Bash scripts\n  /tool append_mode [off|full|summary] - Configure how tool loop results are recorded into chat history (summary: compact redacted summary [default]; full: all intermediate turns; off: only final response)\n  /tool max_turns [count] - Set or show the default maximum turns for autonomous tool loops (e.g. when triggered via '/tool auto on')\n  /tool rate_limit [seconds] - Set or show rate limit delay (seconds) between LLM calls in tool loop\n  /tool prompt [live_edit|edit_live|restore] - Show prompts injected during tool operation, open editor to live-edit system instructions, or restore prompt overrides to tools_config.toml defaults\n  /tool retry [edit|fix|run] [--continue] - Post-turn tool rescue: extract or infer tool calls from LAST_COMPLETION, repair in $EDITOR with parameter schema hints and warnings, or interactively fix in terminal and dispatch. Use --continue to immediately resume agentic loop\n  /tool continue [max_turns] - Continue the agentic loop by handing the rescued tool execution result back to the LLM\n  /tool inject [file=<path>|<payload>] - Inject synthetic or malformed completion into LAST_COMPLETION and session history for offline tool rescue testing\n  /tool loop [turns|max|max=val] [force] - Start the autonomous agentic tool loop. Runs for the specified number of turns (default 25). Use 'max' or 'max=100' to set limits. Loop counts greater than 100 require the 'force' flag.\n  /tool auto [on|off] - Enable/disable auto execution of the tool loop upon detecting a tool call in any completion\n  /tool history [<turn_id>|current] [csv [file.csv]] [var=<name>] - List all agentic tool loops recorded in the active session, or show per-call details for a specific turn. Supports CSV export. Populates protected variable ${TOOL_HISTORY} and optional target variable.\n  /tool export csv <file.csv> - Export all agentic tool loop records across the active session to CSV\n  /tool replay [<turn_id>] [at <N>|diff <A> <B>|step] [limit=<N>] [var=<name>] - Time-travel agentic loop replay: reconstruct the context window the loop saw at each step and run truncation diagnostics. Populates protected variable ${TOOL_REPLAY} and optional target variable.\n  /tool <file.json> - Dispatch tool invocation from JSON file\n  /tool [json] - Dispatch a tool invocation directly (uses LAST_COMPLETION if no argument provided)",
            examples=["/tool on", "/tool off", "/tool list", "/tool list var=my_tools", "/tool auto on", "/tool scratch on", "/tool scratch status", "/tool scratch clean", "/tool append_mode summary", "/tool append_mode full", "/tool append_mode off", "/tool max_turns 75", "/tool rate_limit 3", "/tool enable list_directory", "/tool disable write_file", "/tool disable all", "/tool prompt", "/tool prompt live_edit", "/tool prompt restore", "/tool retry edit", "/tool retry fix", "/tool retry run", "/tool retry run --continue", "/tool continue", "/tool inject <invoke name=\"write_file\"><parameter name=\"path\">test.py</parameter></invoke>", "/tool inject file=crashes/malformed_turn.txt", "/tool loop", "/tool loop 50", "/tool history", "/tool history 3 var=loop_3", "/tool history current", "/tool history csv tools.csv", "/tool export csv tools.csv", "/tool replay", "/tool replay at 2 var=snap2", "/tool replay diff 1 3", "/tool replay step", "/tool replay limit=8000", '/tool {"tool": "list_directory", "arguments": {"path": "."}}'],
            see_also=["/run", "/continue"]
        ))

        self.register_command(CommandHelp(
            name="/continue",
            category="scripting",
            short_desc="Continue agentic loop with last rescued tool execution result",
            usage="/continue [max_turns]",
            long_desc="Hands the most recent rescued tool execution result (${TOOL_DISPATCH_RESULT} / ${LAST_TOOL_RESCUED}) back to the active model and resumes the multi-turn agentic tool loop.\n\nShorthand alias for '/tool continue'.",
            examples=["/continue", "/continue 10"],
            see_also=["/tool"]
        ))
        ...
        self.register_command(CommandHelp(
            ...
            usage="/session [start <name>|new <name>|auto [on|off]|history [on|off]|stop|off|status|note <text>|save [name]|list [all|compressed|uncompressed|limit=N|range=start:end|model=M|var=<name>]|use <name>|show [-t]|export [csv] <file> [-t]|info|delete <name|id|--all>|merge <target> <s1> <s2>|compress [name|pattern|days|all]|uncompress [name|pattern|all]|prune [keep=N] [days=D] [size=M]|replay [<id>] [at <N>|diff <A> <B>|step] [limit=<N>] [var=<name>]|query <terms...> [or|and] [ids] [full] [since=D] [until=D] [range=D to D] [--scratch] [limit=N] [var=<name>]|get <id|name|active> [turn=N|all] [prompt|response|both|thinking] [var=<name>]]",
            long_desc="Manage session-based conversation persistence and workspace storage.\n\nSubcommands:\n  /session start <name> (or /session new <name>) - Clear active history and start a new named session\n  /session auto [on|off] - Toggle automatic session creation & turn saving\n  /session history [on|off] - Toggle in-memory chat history collection (disables tool loops when off)\n  /session stop / off - Pause session recording\n  /session status - Show active session details and file path\n  /session note <text> - Add/update annotation note (max 1024 chars, excluded from LLM context)\n  /session save [name] - Save or update active session custom name\n  /session list [all|compressed|uncompressed|limit=N|range=start:end|model=M|var=<name>] - List saved sessions with prompt slugs, notes, and turn counts (default: show 10 newest). Automatically populates protected variable ${SESSION_LIST} and optional target variable with session records\n  /session use <name|id> - Load an existing session from disk (automatically decompresses if compressed)\n  /session show [--thinking|-t] - Display formatted view of active session\n  /session export [csv] <file> [--thinking|-t] - Export session transcript to Markdown (.md) or structured data to CSV (.csv)\n  /session info / stats - Display workspace metrics (count, total size, oldest, newest, largest)\n  /session delete <name|id|--all> - Delete a specific session file or purge all sessions\n  /session merge <target> <s1> <s2> - Merge multiple sessions sequentially into a new session\n  /session compress [name|pattern|days|all] - Gzip compress session files by name, wildcard (e.g. mistral*), or age\n  /session uncompress [name|pattern|all] - Decompress gzip compressed session files by name, wildcard, or all\n  /session prune [keep=N] [days=D] [size=M] - Prune session files by count, age, or size quota\n  /session replay [<id>] [at <N>|diff <A> <B>|step] [limit=<N>] [var=<name>] - Replay and inspect context window history and truncation state (time-travel context replay). Automatically populates protected variable ${REPLAY} and optional target variable\n  /session query <terms...> [or|and] [ids] [full] [since=D] [until=D] [range=D to D] [--scratch] [limit=N] [var=<name>] - Search across active and saved session turns, notes, and scratchpad files with boolean logic and date filtering. 'ids' returns only session IDs; 'full' outputs unabbreviated turn content. Automatically populates protected variable ${SESSION_QUERY} and optional target variable\n  /session get <id|name|active> [turn=N|all] [prompt|response|both|thinking] [var=<name>] - Non-destructive read-only extraction of turn text from any session without switching sessions. Optionally saves extracted text to target variable",
            examples=["/session start refactor_parser", "/session new debug_tool", "/session list compressed", "/session list model=test_model var=my_sessions", "/session query auth jwt", "/session query bug ids var=hit_sids", "/session query error full since=yesterday", "/session get sess_1 turn=3 response var=code", "/session get active all var=transcript", "/session compress mistral*", "/session compress 7", "/session uncompress *dufu*", "/session uncompress all", "/session history off", "/session note benchmark run for v2.5", "/session info", "/session delete old_session", "/session merge combined_session s1 s2", "/session prune keep=10 days=30 size=50", "/session export summary.md -t", "/session export csv turns.csv", "/session replay", "/session replay at 3 var=snap3", "/session replay diff 2 3", "/session replay limit=16000"],
        ...
        self.register_command(CommandHelp(
            name="/replay",
            category="session",
            short_desc="Time-travel context replay: inspect the reconstructed context window at any turn",
            usage="/replay [<id>] [at <N> | diff <A> <B> | step] [limit=<N>] [var=<name>]",
            long_desc="Replay and inspect session context window history and truncation state. Reconstructs the exact message array sent to the model at each turn and runs verbose truncation diagnostics. Automatically populates protected variable ${REPLAY} and optional target variable.\n\nModes:\n  /replay - Summary timeline of all turns in the active session (Turn #, Uncut/Truncated tokens, Evicted count, Anchor warning)\n  /replay <id> - Summary timeline for session <id>\n  /replay [id] at <N> - Full reconstructed message dump at Turn N (anchors, evicted messages, token counts, previews)\n  /replay [id] diff <A> <B> - Compare Turn A vs Turn B (added messages, newly evicted, token delta)\n  /replay [id] step - Interactive turn-by-turn stepper (Enter=next, q=quit, show=full dump)\n  /replay ... limit=<N> - Override the context limit for any mode (e.g. test a 16k budget)\n  /replay ... var=<name> - Save replay snapshots or diff to custom variable\n\nDefaults to the active session when <id> is omitted. The system prompt is reconstructed from current config as an approximation. /replay is a shorthand alias for /session replay.",
            examples=["/replay", "/replay var=timeline", "/replay at 3 var=snap3", "/replay diff 2 3", "/replay step", "/replay limit=16000", "/replay my_session at 2"],
            see_also=["/session"]
        ))
