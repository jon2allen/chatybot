...
class HelpSystem:
    ...
    def _initialize_commands(self) -> None:
        ...
        self.register_command(CommandHelp(
            ...
            usage="/tool [on|off|list|enable <tool>|disable <tool>|scratch [on|off|clean|status]|max_turns <int>|rate_limit <seconds>|prompt [live_edit|edit_live|restore]|loop [turns|max|max=val] [force]|auto [on|off]|<json_file.json>|<json_invocation>]",
            ...
            examples=["/tool on", "/tool off", "/tool list", "/tool auto on", "/tool scratch on", "/tool scratch status", "/tool scratch clean", "/tool max_turns 75", "/tool rate_limit 3", "/tool enable list_directory", "/tool disable write_file", "/tool disable all", "/tool prompt", "/tool prompt live_edit", "/tool prompt restore", "/tool loop", "/tool loop 50", '/tool {"tool": "list_directory", "arguments": {"path": "."}}'],
            see_also=["/run"]
        ))
