...
class HelpSystem:
    ...
    def _initialize_commands(self) -> None:
        ...
        self.register_command(CommandHelp(
            ...
            usage="/tool [on|off|prompt|loop [turns|max|max=val] [force]|auto [on|off]|<json_file.json>|<json_invocation>]",
            ...
            examples=["/tool on", "/tool off", "/tool prompt", "/tool loop", "/tool loop max", "/tool loop 150 force", "/tool auto on", "/tool auto off", "/tool find1.json", "/tool {\"tool\": \"list_directory\", \"arguments\": {\"path\": \".\"}}"],
            see_also=["/run"]
        ))
