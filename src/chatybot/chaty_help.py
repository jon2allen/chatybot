...
class HelpSystem:
    ...
    def _initialize_commands(self) -> None:
        ...
        self.register_command(CommandHelp(
            ...
            usage="/session [start <name>|auto [on|off]|stop|off|status|note <text>|save [name]|list|use <name>|show [-t]|export <file.md> [-t]|info|delete <name|id|--all>|merge <target> <s1> <s2>|compress [days|all]|prune [keep=N] [days=D] [size=M]]",
            ...
            examples=["/session start refactor_parser", "/session note benchmark run for v2.5", "/session info", "/session delete old_session", "/session merge combined_session s1 s2", "/session compress 7", "/session prune keep=10 days=30 size=50", "/session export summary.md -t"],
