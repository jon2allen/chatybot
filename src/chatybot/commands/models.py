...
async def cmd_model(ctx: CommandContext, parts: list, command: str) -> CommandResult:
    ...
    model_config = app.config_manager.get_model_config(model_alias)
    if hasattr(app, "context_limiter") and not app.context_limiter._user_set_limit:
        app.context_limiter.set_limit(model_config.get("context_limit"), from_user=False)
