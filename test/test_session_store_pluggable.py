...
def test_session_lifecycle(engine_name, temp_sessions_dir):
    ...
    # 6. Save metadata update
    loaded_meta["notes"] = "Updated note"
    store.save_meta("test_sess_001", loaded_meta)
    reloaded_meta, _ = store.load_session("test_sess_001")
    assert reloaded_meta["notes"] == "Updated note"
    ...
    # 9. Load compressed session (verifies auto-decompression)
    c_meta, c_turns = store.load_session("test_sess_001")
    assert len(c_turns) == 2

    # 10. Uncompress with glob wildcard
